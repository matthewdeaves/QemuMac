...
#######################################
# Setup TAP networking with enhanced error handling and NAT/DHCP
# Arguments:
#   None
# Globals:
...
setup_tap_networking() {
    info_log "Setting up TAP networking with DHCP and NAT..."
    ...
    check_command "iptables" "iptables" || exit 1
    ...
    local bridge_ip_cidr="192.168.99.1/24"
    local dhcp_range="192.168.99.100,192.168.99.200"

    # Setup bridge with a static IP
    setup_bridge "$BRIDGE_NAME" "$bridge_ip_cidr"

    # Setup TAP interface for this VM
    setup_tap "$TAP_DEV_NAME" "$BRIDGE_NAME" "$(whoami)"

    # Configure Host for Internet Sharing (NAT) and DHCP
    local cleanup_info
    cleanup_info=$(setup_nat_and_dhcp_for_bridge "$BRIDGE_NAME" "$dhcp_range")

    # Set trap to clean up TAP and NAT/DHCP on exit
    info_log "Cleanup trap set for TAP, NAT, and DHCP."
    # shellcheck disable=SC2064
    trap "cleanup_tap '$TAP_DEV_NAME' '$BRIDGE_NAME'; cleanup_nat_and_dhcp '$cleanup_info'" EXIT SIGINT SIGTERM

    info_log "TAP networking setup complete."
}
...
setup_passt_networking() {
    ...
    passt --dns 8.8.8.8 --socket "$socket_path" --foreground &
    local passt_pid=$!
...
build_network_args() {
    ...
    if [ -n "${QEMU_NETWORK_DEVICE:-}" ]; then
        ...
        network_device="sungem"  # PowerPC default, more compatible with OS X
    else
